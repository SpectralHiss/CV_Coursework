...
[ numTrain , dim] = size(TrainMat);
[ numTest, ~ ] = size(TestMat);

for i = 1:numTrain 
     example = TrainMat(i,:);
     ds = EuclideanDistance(example,C);
     [minv, min_index] = min(ds);
     index_train(i) = min_index;
end
...
wordid = 397; %37;% set a random value and we intead to find 20 patches from both training and test
...
for ii = 1:nimages
      ...
      load(inFName, 'features');
      [ num_feats , ~ ] = size(features.data);
      for row = 1:num_feats
         ds = EuclideanDistance(features.data(row,:),C);
         [minv, min_index] = min(ds);
         BoW(ii,min_index) = BoW(ii,min_index)+1;
      end
...
clear BoW;
k = 5;% set the k for k-nn algorithm 
